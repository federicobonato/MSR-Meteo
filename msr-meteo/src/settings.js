...
function saveSettings() {
  try {
    localStorage.setItem(SETTINGS_KEY, JSON.stringify({ fontSize: currentFontSize, speedOverrides }));
  } catch(e) {}
}
...
function recalcTimes() {
  ...
  for (let i = 0; i < CPS.length - 1; i++) {
    ...
    if (ORIG_CPS[i + 1].arr && ORIG_CPS[i + 1].dep) {
      const stopMs = new Date(ORIG_CPS[i + 1].dep) - new Date(ORIG_CPS[i + 1].arr);
      to.dep = new Date(newArr.getTime() + Math.max(0, stopMs)).toISOString().slice(0, 19);
    } else {
      to.dep = ORIG_CPS[i + 1].dep;
    }
  }
...
function resetSettings() {
  ...
  speedOverrides  = {};
  ORIG_CPS.forEach((orig, idx) => { CPS[idx].arr = orig.arr; CPS[idx].dep = orig.dep; });
  applyFontSize(currentFontSize);
  saveSettings();
  buildCards();
  CPS.forEach(cp => {
