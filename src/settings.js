...
function computeOriginalSpeed(segIdx) {
  return DEFAULT_SPEEDS[segIdx] || 15.0;
}
...
function recalcTimes() {
  ...
  for (let i = 0; i < CPS.length - 1; i++) {
    ...
    const to     = CPS[i + 1];
    const distKm = to.kms;
    const speed  = speedOverrides[i] !== undefined ? speedOverrides[i] : computeOriginalSpeed(i);
    if (!from.dep || distKm <= 0 || speed <= 0) {
      to.arr = null;
      to.dep = null;
      continue;
    }
    const travelMs = (distKm / speed) * 3600000;
    const newArr = new Date(new Date(from.dep).getTime() + travelMs);
    to.arr = newArr.toISOString();
    if (i + 1 === CPS.length - 1) {
      to.dep = null;
    } else {
      const stopMinutes = stopOverrides[to.n] !== undefined ? stopOverrides[to.n] : (to.stopMinutes || 0);
      to.dep = new Date(newArr.getTime() + stopMinutes * 60000).toISOString();
    }
  }
}
